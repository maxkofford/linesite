...
class DB {
     ...
     *
     * @param string $method
     *            What (non-existent) method was called.
     * @param array $args
     ...
     *
     * @param bool $override
     *            Whether or not to always use Master/Write
     */
     ...
     *            See Connection_Type above for valid values.
     * @param array $options
     *            Options to pass to the PDO constructor
     * @return \PDO|null
     */
    ...
    public static function get_pdo($connection_type = Connection_Type::WRITE, $options = array()) {
        ...
        try {
            ...
            $pdo = new \PDO($connection_details['dsn'], $connection_details['user'], $connection_details['password'], $options);
        }
     ...
     * @param bool $use_reporting
     *            Whether or not to use the reporting server
     * @param bool $transactional
     *            Whether or not this is a locking transactional query.
     ...
     *
     * @param string $sql
     *            The select query string that you wish to execute.
     * @param array $parameters
    ...
    public static function execute($sql, $parameters = array(), $object = "", $use_reporting = false, $transactional = false, $error_tag = '') {
        ...
        foreach ($parameters as $field => $parameter) {
            if (is_object($parameter)) {
                ...
                $normalized_parameters[$field] = array(
                    'value' => $parameter,
                    'type' => \PDO::PARAM_STR
                );
            }
     ...
     *
     * @param string $field
     *            Which field we're binding the array to
     * @param string $sql
     *            The query we're doing the bindings on
     * @param array $array
     *            The values that we want bound to individual parameters
     * @param \PDO::value_type $array_type
     *            \PDO::PARAM_STR, \PDO::PARAM_INT, ect
     *            
     ...
     * @param string $sql
     * @param array $parameters
     *            same as run except the first array parameter is batched
     * @param int $batch_size
     *            maximum batch size
     * @param int $second_delay_between_batches
     *            seconds of delay between batches so db logs dont build up too quickly
     */
     ...
     * @param number $biggest_batch_size
     ...
     * @param array $parameters
     *            The parameters used in the $sql query.
     * @param \PDOStatement|bool $prepared_query
     *            The PDO to check errors on.
     * @param string $error_tag
     *            A optional tag to add to errors for this query
     *            
